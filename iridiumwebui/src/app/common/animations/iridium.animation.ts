...
export const IridiumAnimation = trigger('IridiumAnimation', [
  transition(':enter', [
    query('.header-logo', style({ opacity: 0 })),
    query('.navigation', style({ opacity: 0 })),
    query('.container-title', [
        style({ transform: 'translateY(-25px)', opacity: 0 }),
        animate('0.6s cubic-bezier(.75,-0.48,.26,1.52)', style({transform: 'translateY(0)', opacity: 1})),
    ]),
    query('.header-logo', [
    ...
    query('.iridium-page', [
        style({ transform: 'translateY(25px)', opacity: 0 }),
        animate('0.5s cubic-bezier(.75,-0.48,.26,1.52)', style({transform: 'translateY(0)', opacity: 1})),
    ]),
  ]),
  transition(':leave', [
    query('.header-logo', [
